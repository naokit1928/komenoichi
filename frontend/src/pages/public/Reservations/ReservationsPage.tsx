...
export default function ReservationsPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (!idData.is_logged_in) {
          navigate("/login-only?redirect=/reservations", { replace: true });
          return;
        }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", backgroundColor: C.bgBase, display: "flex", justifyContent: "center", alignItems: "center" }}>
        <div style={{ color: C.ink3, fontWeight: 600 }}>読み込み中...</div>
        <PublicBottomBar consumerEmail={null} />
      </div>
    );
  }
              ...
              {upcomingItems.length > 0 ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {upcomingItems.map((upcomingItem) => (
                    ...
                    >
                      <div style={{ display: "inline-block", backgroundColor: C.bgPale, color: C.ink2, fontSize: 11, fontWeight: 600, padding: "4px 8px", borderRadius: 9999, marginBottom: 12 }}>
                        受け取り待ち
                      </div>
          ...
          {activeTab === "past" && (
            <div>
              {pastItems.length > 0 ? (
                  ...
                  {pastItems.map((res) => (
                    <div key={res.reservation_id} style={{ backgroundColor: "#fff", border: `1px solid ${C.border}`, borderRadius: 16, padding: 16 }}>
                      ...
                      {res.status_category === "completed" && (
                        <button
                          onClick={() => navigate(`/farms/${res.farm_id}`)}
                          style={{ width: "100%", padding: "10px 0", borderRadius: 9999, backgroundColor: "#fff", color: C.ink2, border: `1px solid ${C.border}`, fontWeight: 600, fontSize: 13, cursor: "pointer", marginTop: 8 }}
                        >
                          もう一度この農家を見る
                        </button>
                      )}
                ...
                <div style={{ textAlign: "center", padding: "48px 0", color: C.ink3, fontSize: 14 }}>
                  過去の履歴はまだありません。
                </div>
              )}
            </div>
