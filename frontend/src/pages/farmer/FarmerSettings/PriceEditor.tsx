// frontend/src/features/farmer-settings/PriceEditor.tsx
// （上部の imports や calc ロジックは変更なし）
...
// ※ ここにあった Toast コンポーネントは削除しました（親で共通管理するため）
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between">
          <div className="text-gray-800" style={{ fontSize: 16, fontWeight: 700, letterSpacing: ".01em" }}>
            10kgの価格
          </div>
          <button aria-label="閉じる" onClick={onClose} style={{ background: "transparent", border: "none", padding: 0, cursor: "pointer" }}>
        ...
        <div className="mt-5 w-full flex justify-center">
          <div style={{ display: "inline-flex", alignItems: "baseline", gap: 0 }}>
            <span className="text-black select-none" style={{ fontWeight: 900, fontSize: 56, lineHeight: 1.15, letterSpacing: "-.01em" }}>
              ￥
            </span>
            <span
              ref={editableRef}
              ...
              onInput={onEdit}
              inputMode="numeric"
              style={{ minWidth: "0.3em", fontWeight: 900, fontSize: 56, lineHeight: 1.15, letterSpacing: "-.01em", outline: "none", border: "none", whiteSpace: "pre" }}
        ...
        <div className="mt-4 text-center space-y-1">
          <div className="text-gray-800">
            ...
            <span className="font-extrabold text-[20px]">￥ {preview5.toLocaleString("ja-JP")}</span>
          </div>
          <div className="text-gray-800">
            <span className="font-semibold text-base">25kg：</span>
            <span className="font-extrabold text-[20px]">￥ {preview25.toLocaleString("ja-JP")}</span>
          </div>
        </div>

        <p className="text-center text-gray-500" style={{ marginTop: 16, marginBottom: 28, fontSize: 12.5, lineHeight: 1.7 }}>
          5kg/25kgは10kgの値段をもとに自動設定されます<br />
          10kgの価格の入力範囲は5,000〜9,900円です<br />
        ...
        <div>
          <button
            ...
            aria-label="保存"
            style={{
              width: "100%", height: 60, background: "#000000", color: "#FFFFFF", borderRadius: 16, fontSize: 22, fontWeight: 700, letterSpacing: 0, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 0 rgba(0,0,0,.02)", transform: "translateZ(0)", opacity: canSave ? 1 : 0.5, cursor: canSave ? "pointer" : "not-allowed",
            }}
...
export default function PriceEditor({
  ...
  async function savePrice10(explicitValue?: number) {
    ...
    if (!Number.isFinite(n)) {
      onError?.("10kgの価格を数値で入力してください。");
      return;
    }
    if (n < minPrice || n > maxPrice) {
      onError?.(`価格は ${minPrice.toLocaleString("ja-JP")}〜${maxPrice.toLocaleString("ja-JP")} 円の範囲で入力してください。`);
      return;
    }
    ...
    try {
      ...
      const normalized10 = typeof data.price_10kg === "number" ? data.price_10kg : n;
      setPrice10Input(normalized10);
      lastSavedRef.current = normalized10;

      if (typeof data.price_5kg === "number") setD5(data.price_5kg);
      if (typeof data.price_25kg === "number") setD25(data.price_25kg);

      onSaved?.(); // ← ここで親に成功を通知し、親側でToastを表示する
      setOpenModal(false);
    } catch (e) {
      console.error(e);
      onError?.("価格の保存に失敗しました。");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="mb-8 mt-6">
      ...
      >
        <div className="text-gray-700" style={{ fontSize: 16, letterSpacing: ".01em", lineHeight: 1.6, fontWeight: 700 }}>
          10kgの価格
        </div>

        <div className="text-black" style={{ fontWeight: 800, fontSize: 44, lineHeight: 1.2, letterSpacing: "-.01em", marginTop: -4 }}>
        ...
        <div className="text-gray-800" style={{ marginTop: 12 }}>
          <span className="inline-flex items-baseline gap-1 font-semibold">
            5kg <b className="font-extrabold text-[18px]">{typeof d5 === "number" ? d5.toLocaleString("ja-JP") : "-"}</b> 円
          </span>
          <span className="mx-2 text-gray-400">／</span>
          <span className="inline-flex items-baseline gap-1 font-semibold">
            25kg <b className="font-extrabold text-[18px]">{typeof d25 === "number" ? d25.toLocaleString("ja-JP") : "-"}</b> 円
          </span>
        </div>
      </button>
