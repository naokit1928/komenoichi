...
# ★ さきほど作成した Service をインポート（パスは実際の配置に合わせてください）
...
def checkout_from_confirm(
    ...
    conn: sqlite3.Connection = Depends(get_db_conn)  # ★ ここでDB接続を自動注入
):
    """
    ...
    # 3. active reservation ガード（業務409）
    ...
    # 4. PENDING 予約の取得 または JIT生成 (Serviceへ完全委譲)
    prep_service = CheckoutPrepService()
    try:
        # conn は Depends で注入されたものを渡すだけ。close() はFastAPIが自動でやってくれる
        ...
        # Service で投げられた ValueError を HTTPエラーに変換
        error_msg = str(e)
        status_code = 400
